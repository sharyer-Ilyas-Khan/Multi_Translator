...
class DictionaryController extends GetxController {
  ...
  Future<void> lookUp() async {
    try {
      String url =
          "https://api.dictionaryapi.dev/api/v2/entries/en/${Uri.encodeFull(inputText.value)}";
      ...
      if (response.statusText == "Not Found") {
        ...
        if (response.body[0]["phonetics"].isNotEmpty) {
          pronounceText.value =
              response.body[0]["phonetics"][0]["text"].toString();
        }

        // word meanings
        partOfSpeech.value =
            response.body[0]["meanings"][0]["partOfSpeech"].toString();
        meaningFirst.value = response.body[0]["meanings"][0]["definitions"][0]
                ["definition"]
            .toString();
        synonyms.value = response.body[0]["meanings"][0]["synonyms"];
        antonyms.value = response.body[0]["meanings"][0]["antonyms"];

        meaningSecond.value = response.body[0]["meanings"][0]["definitions"][1]
                ["definition"]
            .toString();
        meaningThird.value = response.body[0]["meanings"][0]["definitions"][2]
                ["definition"]
            .toString();
