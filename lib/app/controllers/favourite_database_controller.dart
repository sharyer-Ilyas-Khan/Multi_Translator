...
class FavouriteDatabaseController extends GetxController {
  ...
  //table name = UniFav
  String createUniFav =
      "CREATE TABLE uni_fav (id INTEGER PRIMARY KEY, 'from' TEXT, 'from_data' TEXT, 'to' TEXT, 'to_data' TEXT)";
  String readUniFav = "SELECT * FROM uni_fav";
  String insertUniFav =
      "INSERT INTO uni_fav('from' , 'from_data', 'to','to_data') VALUES('eng','abc','ur','alif')";
  String deleteUniFav = "DELETE FROM uni_fav WHERE id = 1";
  //voice queries
  //table name = VoiceFav
  String createVoiceFav =
      "CREATE TABLE voice_fav (id INTEGER PRIMARY KEY, 'from' TEXT, 'from_data' TEXT, 'to' TEXT, 'to_data' TEXT)";
  String readVoiceFav = "SELECT * FROM voice_fav";
  String insertVoiceFav =
      "INSERT INTO voice_fav('from', 'from_data', 'to','to_data') VALUES('eng','abc','ur','alif')";
  String deleteVoiceFav = "DELETE FROM voice_fav WHERE id = 1";
  //multi queries
  //table name = MultiFav
  String createMultiFav =
      "CREATE TABLE multi_fav (id INTEGER PRIMARY KEY, 'from' TEXT, 'from_data' TEXT, 'to' LIST, 'to_data' LIST)";
  String readMultiFav = "SELECT * FROM multi_fav";
  String insertMultiFav =
      "INSERT INTO multi_fav('from', 'from_data', 'to','to_data') VALUES('eng','abc','[ur,GR]','[alif,'JE']')";
  ...
  void addToUniFavourite(from, fromData, to, toData) async {
    await db!.rawInsert(
        "INSERT INTO uni_fav ('from', 'from_data', 'to', 'to_data') VALUES('$from','$fromData','$to','$toData')");
  }
  ...
  void addToVoiceFavourite(from, fromData, to, toData) async {
    await db!.rawInsert(
        "INSERT INTO voice_fav('from', 'from_data', 'to','to_data') VALUES('$from','$fromData','$to','$toData')");
  }
  ...
  void addToMultiFavourite(from, fromData, toList, toDataList) async {
    await db!.rawInsert(
        'INSERT INTO multi_fav("from", "from_data", "to","to_data") VALUES("$from","$fromData","$toList","$toDataList")');
  }
  ...
  Future<String> getId(type, fromData) async {
    String id = "";
    await db!
        .rawQuery("SELECT id FROM ${type}_fav Where from_data== '${fromData}'")
        .then((value) {
